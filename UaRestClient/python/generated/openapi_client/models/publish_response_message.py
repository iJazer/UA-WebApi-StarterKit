...
class PublishResponseMessage(BaseModel):
    """
    ...
    body: Optional[PublishResponse] = Field(None, alias="Body")
    ...
    def service_id_validate_enum(cls, value):
        ...
        if value not in (0, 827, 395):
            raise ValueError("must be one of enum values (0, 827, 395)")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PublishResponseMessage:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> PublishResponseMessage:
        """Create an instance of PublishResponseMessage from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PublishResponseMessage.parse_obj(obj)

        _obj = PublishResponseMessage.parse_obj({
            "namespace_uris": obj.get("NamespaceUris"),
            "server_uris": obj.get("ServerUris"),
            "service_id": obj.get("ServiceId"),
            "body": PublishResponse.from_dict(obj.get("Body")) if obj.get("Body") is not None else None
        })
        return _obj
