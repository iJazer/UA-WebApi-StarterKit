...
class CloseSessionResponseMessage(BaseModel):
    """
    ...
    body: Optional[CloseSessionResponse] = Field(None, alias="Body")
    ...
    def service_id_validate_enum(cls, value):
        ...
        if value not in (0, 474, 395):
            raise ValueError("must be one of enum values (0, 474, 395)")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CloseSessionResponseMessage:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CloseSessionResponseMessage:
        """Create an instance of CloseSessionResponseMessage from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CloseSessionResponseMessage.parse_obj(obj)

        _obj = CloseSessionResponseMessage.parse_obj({
            "namespace_uris": obj.get("NamespaceUris"),
            "server_uris": obj.get("ServerUris"),
            "service_id": obj.get("ServiceId"),
            "body": CloseSessionResponse.from_dict(obj.get("Body")) if obj.get("Body") is not None else None
        })
        return _obj
