...
class ActivateSessionResponseMessage(BaseModel):
    """
    ...
    body: Optional[ActivateSessionResponse] = Field(None, alias="Body")
    ...
    def service_id_validate_enum(cls, value):
        ...
        if value not in (0, 468, 395):
            raise ValueError("must be one of enum values (0, 468, 395)")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ActivateSessionResponseMessage:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ActivateSessionResponseMessage:
        """Create an instance of ActivateSessionResponseMessage from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ActivateSessionResponseMessage.parse_obj(obj)

        _obj = ActivateSessionResponseMessage.parse_obj({
            "namespace_uris": obj.get("NamespaceUris"),
            "server_uris": obj.get("ServerUris"),
            "service_id": obj.get("ServiceId"),
            "body": ActivateSessionResponse.from_dict(obj.get("Body")) if obj.get("Body") is not None else None
        })
        return _obj
