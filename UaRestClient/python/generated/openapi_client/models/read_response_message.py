...
class ReadResponseMessage(BaseModel):
    """
    ReadResponseMessage
    """
    ...
    body: Optional[ReadResponse] = Field(None, alias="Body")
    ...
    def service_id_validate_enum(cls, value):
        ...
        if value not in (0, 632, 395):
            raise ValueError("must be one of enum values (0, 632, 395)")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ReadResponseMessage:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ReadResponseMessage:
        """Create an instance of ReadResponseMessage from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ReadResponseMessage.parse_obj(obj)

        _obj = ReadResponseMessage.parse_obj({
            "namespace_uris": obj.get("NamespaceUris"),
            "server_uris": obj.get("ServerUris"),
            "service_id": obj.get("ServiceId"),
            "body": ReadResponse.from_dict(obj.get("Body")) if obj.get("Body") is not None else None
        })
        return _obj
