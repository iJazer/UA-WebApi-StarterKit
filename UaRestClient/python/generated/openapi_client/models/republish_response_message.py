...
class RepublishResponseMessage(BaseModel):
    """
    ...
    body: Optional[RepublishResponse] = Field(None, alias="Body")
    ...
    def service_id_validate_enum(cls, value):
        ...
        if value not in (0, 833, 395):
            raise ValueError("must be one of enum values (0, 833, 395)")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RepublishResponseMessage:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> RepublishResponseMessage:
        """Create an instance of RepublishResponseMessage from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RepublishResponseMessage.parse_obj(obj)

        _obj = RepublishResponseMessage.parse_obj({
            "namespace_uris": obj.get("NamespaceUris"),
            "server_uris": obj.get("ServerUris"),
            "service_id": obj.get("ServiceId"),
            "body": RepublishResponse.from_dict(obj.get("Body")) if obj.get("Body") is not None else None
        })
        return _obj
