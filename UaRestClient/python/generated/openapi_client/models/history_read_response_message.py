...
class HistoryReadResponseMessage(BaseModel):
    """
    ...
    body: Optional[HistoryReadResponse] = Field(None, alias="Body")
    ...
    def service_id_validate_enum(cls, value):
        ...
        if value not in (0, 665, 395):
            raise ValueError("must be one of enum values (0, 665, 395)")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> HistoryReadResponseMessage:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> HistoryReadResponseMessage:
        """Create an instance of HistoryReadResponseMessage from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return HistoryReadResponseMessage.parse_obj(obj)

        _obj = HistoryReadResponseMessage.parse_obj({
            "namespace_uris": obj.get("NamespaceUris"),
            "server_uris": obj.get("ServerUris"),
            "service_id": obj.get("ServiceId"),
            "body": HistoryReadResponse.from_dict(obj.get("Body")) if obj.get("Body") is not None else None
        })
        return _obj
