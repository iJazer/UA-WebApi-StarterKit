...
class FindServersResponse(BaseModel):
    """
    FindServersResponse
    """
    response_header: Optional[ResponseHeader] = Field(None, alias="ResponseHeader")
    servers: Optional[conlist(ApplicationDescription)] = Field(None, alias="Servers")
    __properties = ["ResponseHeader", "Servers"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> FindServersResponse:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> FindServersResponse:
        """Create an instance of FindServersResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return FindServersResponse.parse_obj(obj)

        _obj = FindServersResponse.parse_obj({
            "response_header": ResponseHeader.from_dict(obj.get("ResponseHeader")) if obj.get("ResponseHeader") is not None else None,
            "servers": [ApplicationDescription.from_dict(_item) for _item in obj.get("Servers")] if obj.get("Servers") is not None else None
        })
        return _obj
