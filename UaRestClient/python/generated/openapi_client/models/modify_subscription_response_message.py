...
class ModifySubscriptionResponseMessage(BaseModel):
    """
    ...
    body: Optional[ModifySubscriptionResponse] = Field(None, alias="Body")
    ...
    def service_id_validate_enum(cls, value):
        ...
        if value not in (0, 794, 395):
            raise ValueError("must be one of enum values (0, 794, 395)")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ModifySubscriptionResponseMessage:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ModifySubscriptionResponseMessage:
        """Create an instance of ModifySubscriptionResponseMessage from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ModifySubscriptionResponseMessage.parse_obj(obj)

        _obj = ModifySubscriptionResponseMessage.parse_obj({
            "namespace_uris": obj.get("NamespaceUris"),
            "server_uris": obj.get("ServerUris"),
            "service_id": obj.get("ServiceId"),
            "body": ModifySubscriptionResponse.from_dict(obj.get("Body")) if obj.get("Body") is not None else None
        })
        return _obj
