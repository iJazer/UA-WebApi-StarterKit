# coding: utf-8

"""
    OPC UA REST API

    This API provides simple REST based access to an OPC UA server.

    The version of the OpenAPI document: 1.05.4
    Contact: office@opcfoundation.org
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from openapi_client.models.pub_sub_configuration2_data_type import PubSubConfiguration2DataType  # noqa: E501

class TestPubSubConfiguration2DataType(unittest.TestCase):
    """PubSubConfiguration2DataType unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PubSubConfiguration2DataType:
        """Test PubSubConfiguration2DataType
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PubSubConfiguration2DataType`
        """
        model = PubSubConfiguration2DataType()  # noqa: E501
        if include_optional:
            return PubSubConfiguration2DataType(
                subscribed_data_sets = [
                    openapi_client.models.standalone_subscribed_data_set_data_type.StandaloneSubscribedDataSetDataType(
                        name = '', 
                        data_set_folder = [
                            ''
                            ], 
                        data_set_meta_data = openapi_client.models.data_set_meta_data_type.DataSetMetaDataType(
                            name = '', 
                            description = openapi_client.models.localized_text.LocalizedText(
                                locale = '', 
                                text = '', ), 
                            fields = [
                                openapi_client.models.field_meta_data.FieldMetaData(
                                    name = '', 
                                    field_flags = 0, 
                                    built_in_type = 0, 
                                    data_type = '', 
                                    value_rank = 56, 
                                    array_dimensions = [
                                        0
                                        ], 
                                    max_string_length = 0, 
                                    data_set_field_id = '', 
                                    properties = [
                                        openapi_client.models.key_value_pair.KeyValuePair(
                                            key = '', 
                                            value = openapi_client.models.variant.Variant(
                                                type = 0, 
                                                body = null, 
                                                dimensions = [
                                                    0
                                                    ], ), )
                                        ], )
                                ], 
                            data_set_class_id = '', 
                            configuration_version = openapi_client.models.configuration_version_data_type.ConfigurationVersionDataType(
                                major_version = 0, 
                                minor_version = 0, ), ), 
                        subscribed_data_set = openapi_client.models.extension_object.ExtensionObject(
                            type_id = '', 
                            encoding = 0, 
                            body = openapi_client.models.body.Body(), ), )
                    ],
                data_set_classes = [
                    openapi_client.models.data_set_meta_data_type.DataSetMetaDataType(
                        name = '', 
                        description = openapi_client.models.localized_text.LocalizedText(
                            locale = '', 
                            text = '', ), 
                        fields = [
                            openapi_client.models.field_meta_data.FieldMetaData(
                                name = '', 
                                field_flags = 0, 
                                built_in_type = 0, 
                                data_type = '', 
                                value_rank = 56, 
                                array_dimensions = [
                                    0
                                    ], 
                                max_string_length = 0, 
                                data_set_field_id = '', 
                                properties = [
                                    openapi_client.models.key_value_pair.KeyValuePair(
                                        key = '', 
                                        value = openapi_client.models.variant.Variant(
                                            type = 0, 
                                            body = null, 
                                            dimensions = [
                                                0
                                                ], ), )
                                    ], )
                            ], 
                        data_set_class_id = '', 
                        configuration_version = openapi_client.models.configuration_version_data_type.ConfigurationVersionDataType(
                            major_version = 0, 
                            minor_version = 0, ), )
                    ],
                default_security_key_services = [
                    openapi_client.models.endpoint_description.EndpointDescription(
                        endpoint_url = '', 
                        server = openapi_client.models.application_description.ApplicationDescription(
                            application_uri = '', 
                            product_uri = '', 
                            application_name = openapi_client.models.localized_text.LocalizedText(
                                locale = '', 
                                text = '', ), 
                            application_type = 56, 
                            gateway_server_uri = '', 
                            discovery_profile_uri = '', 
                            discovery_urls = [
                                ''
                                ], ), 
                        server_certificate = 'YQ==', 
                        security_mode = 56, 
                        security_policy_uri = '', 
                        user_identity_tokens = [
                            openapi_client.models.user_token_policy.UserTokenPolicy(
                                policy_id = '', 
                                token_type = 56, 
                                issued_token_type = '', 
                                issuer_endpoint_url = '', 
                                security_policy_uri = '', )
                            ], 
                        transport_profile_uri = '', 
                        security_level = 0, )
                    ],
                security_groups = [
                    openapi_client.models.security_group_data_type.SecurityGroupDataType(
                        name = '', 
                        security_group_folder = [
                            ''
                            ], 
                        key_lifetime = 1.337, 
                        security_policy_uri = '', 
                        max_future_key_count = 0, 
                        max_past_key_count = 0, 
                        security_group_id = '', 
                        role_permissions = [
                            openapi_client.models.role_permission_type.RolePermissionType(
                                role_id = '', 
                                permissions = 0, )
                            ], 
                        group_properties = [
                            openapi_client.models.key_value_pair.KeyValuePair(
                                key = '', 
                                value = openapi_client.models.variant.Variant(
                                    type = 0, 
                                    body = null, 
                                    dimensions = [
                                        0
                                        ], ), )
                            ], )
                    ],
                pub_sub_key_push_targets = [
                    openapi_client.models.pub_sub_key_push_target_data_type.PubSubKeyPushTargetDataType(
                        application_uri = '', 
                        push_target_folder = [
                            ''
                            ], 
                        endpoint_url = '', 
                        security_policy_uri = '', 
                        user_token_type = openapi_client.models.user_token_policy.UserTokenPolicy(
                            policy_id = '', 
                            token_type = 56, 
                            issued_token_type = '', 
                            issuer_endpoint_url = '', 
                            security_policy_uri = '', ), 
                        requested_key_count = 0, 
                        retry_interval = 1.337, 
                        push_target_properties = [
                            openapi_client.models.key_value_pair.KeyValuePair(
                                key = '', 
                                value = openapi_client.models.variant.Variant(
                                    type = 0, 
                                    body = null, 
                                    dimensions = [
                                        0
                                        ], ), )
                            ], 
                        security_groups = [
                            ''
                            ], )
                    ],
                configuration_version = 0,
                configuration_properties = [
                    openapi_client.models.key_value_pair.KeyValuePair(
                        key = '', 
                        value = openapi_client.models.variant.Variant(
                            type = 0, 
                            body = null, 
                            dimensions = [
                                0
                                ], ), )
                    ],
                published_data_sets = [
                    openapi_client.models.published_data_set_data_type.PublishedDataSetDataType(
                        name = '', 
                        data_set_folder = [
                            ''
                            ], 
                        data_set_meta_data = openapi_client.models.data_set_meta_data_type.DataSetMetaDataType(
                            name = '', 
                            description = openapi_client.models.localized_text.LocalizedText(
                                locale = '', 
                                text = '', ), 
                            fields = [
                                openapi_client.models.field_meta_data.FieldMetaData(
                                    name = '', 
                                    field_flags = 0, 
                                    built_in_type = 0, 
                                    data_type = '', 
                                    value_rank = 56, 
                                    array_dimensions = [
                                        0
                                        ], 
                                    max_string_length = 0, 
                                    data_set_field_id = '', 
                                    properties = [
                                        openapi_client.models.key_value_pair.KeyValuePair(
                                            key = '', 
                                            value = openapi_client.models.variant.Variant(
                                                type = 0, 
                                                body = null, 
                                                dimensions = [
                                                    0
                                                    ], ), )
                                        ], )
                                ], 
                            data_set_class_id = '', 
                            configuration_version = openapi_client.models.configuration_version_data_type.ConfigurationVersionDataType(
                                major_version = 0, 
                                minor_version = 0, ), ), 
                        extension_fields = [
                            openapi_client.models.key_value_pair.KeyValuePair(
                                key = '', )
                            ], 
                        data_set_source = openapi_client.models.extension_object.ExtensionObject(
                            type_id = '', 
                            encoding = 0, 
                            body = openapi_client.models.body.Body(), ), )
                    ],
                connections = [
                    openapi_client.models.pub_sub_connection_data_type.PubSubConnectionDataType(
                        name = '', 
                        enabled = True, 
                        publisher_id = openapi_client.models.variant.Variant(
                            type = 0, 
                            body = null, 
                            dimensions = [
                                0
                                ], ), 
                        transport_profile_uri = '', 
                        address = openapi_client.models.extension_object.ExtensionObject(
                            type_id = '', 
                            encoding = 0, 
                            body = openapi_client.models.body.Body(), ), 
                        connection_properties = [
                            openapi_client.models.key_value_pair.KeyValuePair(
                                key = '', 
                                value = openapi_client.models.variant.Variant(
                                    type = 0, 
                                    body = null, ), )
                            ], 
                        transport_settings = openapi_client.models.extension_object.ExtensionObject(
                            type_id = '', 
                            encoding = 0, 
                            body = openapi_client.models.body.Body(), ), 
                        writer_groups = [
                            openapi_client.models.writer_group_data_type.WriterGroupDataType(
                                writer_group_id = 0, 
                                publishing_interval = 1.337, 
                                keep_alive_time = 1.337, 
                                priority = 0, 
                                locale_ids = [
                                    ''
                                    ], 
                                header_layout_uri = '', 
                                message_settings = , 
                                data_set_writers = [
                                    openapi_client.models.data_set_writer_data_type.DataSetWriterDataType(
                                        name = '', 
                                        enabled = True, 
                                        data_set_writer_id = 0, 
                                        data_set_field_content_mask = 0, 
                                        key_frame_count = 0, 
                                        data_set_name = '', 
                                        data_set_writer_properties = [
                                            openapi_client.models.key_value_pair.KeyValuePair(
                                                key = '', )
                                            ], )
                                    ], )
                            ], 
                        reader_groups = [
                            openapi_client.models.reader_group_data_type.ReaderGroupDataType(
                                data_set_readers = [
                                    openapi_client.models.data_set_reader_data_type.DataSetReaderDataType(
                                        name = '', 
                                        enabled = True, 
                                        writer_group_id = 0, 
                                        data_set_writer_id = 0, 
                                        data_set_meta_data = openapi_client.models.data_set_meta_data_type.DataSetMetaDataType(
                                            name = '', 
                                            description = openapi_client.models.localized_text.LocalizedText(
                                                locale = '', 
                                                text = '', ), 
                                            fields = [
                                                openapi_client.models.field_meta_data.FieldMetaData(
                                                    name = '', 
                                                    field_flags = 0, 
                                                    built_in_type = 0, 
                                                    data_type = '', 
                                                    value_rank = 56, 
                                                    array_dimensions = [
                                                        0
                                                        ], 
                                                    max_string_length = 0, 
                                                    data_set_field_id = '', 
                                                    properties = [
                                                        
                                                        ], )
                                                ], 
                                            data_set_class_id = '', 
                                            configuration_version = openapi_client.models.configuration_version_data_type.ConfigurationVersionDataType(
                                                major_version = 0, 
                                                minor_version = 0, ), ), 
                                        data_set_field_content_mask = 0, 
                                        message_receive_timeout = 1.337, 
                                        key_frame_count = 0, 
                                        header_layout_uri = '', 
                                        security_mode = 56, 
                                        security_group_id = '', 
                                        security_key_services = [
                                            openapi_client.models.endpoint_description.EndpointDescription(
                                                endpoint_url = '', 
                                                server = openapi_client.models.application_description.ApplicationDescription(
                                                    application_uri = '', 
                                                    product_uri = '', 
                                                    application_name = openapi_client.models.localized_text.LocalizedText(
                                                        locale = '', 
                                                        text = '', ), 
                                                    application_type = 56, 
                                                    gateway_server_uri = '', 
                                                    discovery_profile_uri = '', 
                                                    discovery_urls = [
                                                        ''
                                                        ], ), 
                                                server_certificate = 'YQ==', 
                                                security_mode = 56, 
                                                security_policy_uri = '', 
                                                user_identity_tokens = [
                                                    openapi_client.models.user_token_policy.UserTokenPolicy(
                                                        policy_id = '', 
                                                        token_type = 56, 
                                                        issued_token_type = '', 
                                                        issuer_endpoint_url = '', 
                                                        security_policy_uri = '', )
                                                    ], 
                                                transport_profile_uri = '', 
                                                security_level = 0, )
                                            ], 
                                        data_set_reader_properties = [
                                            
                                            ], 
                                        subscribed_data_set = , )
                                    ], )
                            ], )
                    ],
                enabled = True
            )
        else:
            return PubSubConfiguration2DataType(
        )
        """

    def testPubSubConfiguration2DataType(self):
        """Test PubSubConfiguration2DataType"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
